...
function MusicView() {
  ...
  const handleClickPrevious = () => {
    setConfig({
      ...config,
      currentMusicIndex:
        config.currentMusicIndex === 0
          ? playlist.length - 1
          : config.currentMusicIndex - 1,
    });
  };
  const handleClickNext = () => {
    setConfig({
      currentMusicIndex:
        config.currentMusicIndex < playlist.length - 1
          ? config.currentMusicIndex + 1
          : 0,
    });
  };
    ...
    <>
      ...
      <div className="home-page--root">
        <div className="music-page">
          <img
            alt="pic"
            src={playlist[config.currentMusicIndex].img}
            className="musicimg"
          />
          <span>{playlist[config.currentMusicIndex].name}</span>
          <span className="span">
            {playlist[config.currentMusicIndex].author}
          </span>
          <AudioPlayer
            style={{
              width: "500px",
